...
| 19 | rewarded-ad credits | DEPLOYED_NOT_EXTERNALLY_VERIFIED | reward intents/status, SSV, quota lots | AdMob duplicate/signature/cap/expiry tests | Existing reward records | Ads + reward + SSV + App Attest + no emergency | Reward card visible when runtime prerequisites pass | Test-only simulation path | Production capability enabled | No genuine production SSV packet | Run and record real Google callback, duplicate, late, and cap evidence |
...
| 22 | App Store Server Notifications | BROKEN_OR_REGRESSED | `apple-notifications-v2.ts`, signed-data verifier | notification ordering/replay/JWS tests | `0011` | Apple environment/credentials, numeric Apple app ID, and endpoint | No direct surface | Invalid JWS controlled rejection | Endpoint deployed without required `APPLE_APP_ID` | Local repair prepared; no real console delivery | Deploy exact repair, then confirm App Store Connect delivery and validate signed events |
...
| 27 | App Attest | LOCAL_VALIDATED_NOT_DEPLOYED | challenge, built-in Apple-root attestation/assertion verifier | identity replay/path/body/counter, synthetic assertion, and official Apple vector tests | `0012`, `0016`, pending `0018` | Apple App Attest service plus exact Team/App/build/environment metadata | Debug targets test; Release targets production | Signed physical Debug artifact, synthetic ECDSA assertion, and Apple's published attestation vector pass | Current runtime still lacks verifier | Real-device server canary absent | Apply `0018`, deploy exact test candidate, then run physical canary |
| 28 | challenge and assertion replay protection | LOCAL_VALIDATED_NOT_DEPLOYED | consumed challenge, expected client-data hash, Apple public-key verification, assertion counter | replay, expiry, wrong path/body, signature, App ID, counter tests | `0012`, `0016`, pending `0018` | Built-in App Attest verifier | API client binds method/path/body | Local focused and cryptographic tests pass | Old final candidate deployed; repair local | Adversarial local proof only | Remote real-device canary required |
...
| 37 | rewarded-ad UI | DEPLOYED_NOT_EXTERNALLY_VERIFIED | reward service/config and `CreditView` | AppModel reward visibility/return tests | None | ads/reward/SSV/App Attest/environment | Card remains visible through temporary attestation recovery; action remains server-gated | Test simulations only | Production capability enabled | No genuine production SSV packet | Validate real callback and return flow |
