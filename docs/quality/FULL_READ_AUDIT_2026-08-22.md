...
# O. 口語ベンチが掘り当てた意図層の穴と、その修正(2026-08-22)

ユーザー指摘「質問文が人間っぽくない」から始めた A/B(綺麗版 vs 口語版、同一8銘柄×同一4意図)で、
**意図分類が口語を認識できない**ことが実測で出た。Q01「この会社ってなにで稼いでんの？」で
**6/8銘柄が deterministic → fallback に転落**。AAPL は同じ filing で
「収益源を十分に特定できません」、JPM は「総収益を源泉にして活動しています」(同語反復)。
本番プロンプト自身が「なにで稼いでんの」を business-model と名指ししているのに、
Worker の意図層が学んでいなかった — 層の間の認識割れ。②削除による退行ではなく、
意図で弾かれると旧定数表にも到達しなかったため、**本番でも従来から踏まれていた穴**。

## 修正(commit ee11b11)

business-overview 認識のコピーが **3か所ではなく6か所**にあった(isBankLike 病の再発)。
`business-overview-question.ts` に共有認識器を1本化し、site 固有の広がりは OR で上乗せ。
非捕獲制約: 「稼」単体は信号にせず手段疑問詞(なにで/どうやって)を要求、
cash-topic veto で「キャッシュフロー稼げてる？」は cash_flow に残す。

受け入れテストは **core-12 ⇔ human-phrasing-12 のペア一致**(45件)。
business-overview の boolean 一致は 12/12。enum 全体では6ペアに既知ギャップが残り、
テスト内に pin して封じてある(将来「吸収して一致」に化けたら落ちる)。

## 実測(run: 2026-08-22-human-q01-refix-8)

| | 修正前(口語) | 修正後(口語) |
|---|---|---|
| AAPL / GOOG | 「特定できません」 | **綺麗版と同一の実セグメント回答** |
| JPM / MA / AVGO / META | 空回答 or 常套句 | deterministic 復帰(ただし薄ラベル、下記) |
| CAT / BAC | fallback(良回答) | 同左 — guard が薄い一次回答を検知しモデルへ、設計どおり |

## 残タスク(v2 品質バックログ、完全性の問題ではない)

1. **Q01 薄ラベル**: JPM/MA「決済・取引サービス」、AVGO「クラウドサービス」、META「その他収益」。
   綺麗版・口語版の両方で同じ = 抽出側の問題。guard の閾値が「短いが具体的に見える誤要約」を
   拾えていない。CAT/BAC で guard が正しく動いた時の回答の方が良い、が改善の方向性のヒント
