...
# Stage 2 途中: 損益表は読めるようになった。**ただし通貨で行き止まりがある**

`workers/src/lib/filings/quarterly-results.ts` を実装し、TSMC と ASML の実物で確認した。
...
| | 見出し | 当期の位置 |
|---|---|---|
| TSMC | `2Q26 \| 2Q25 \| YoY% \| 1Q26 \| QoQ%` | **左端** |
| ASML | `Q1 2026 \| Q2 2026` | **右端** |

「最初の数値列」を採る素直な実装だと、**ASML では前四半期(8,767)を今期として出す**
(正しくは 9,326)。見出し行に期が書いてあるので、**期の一致で列を決める**ようにした。
見出しに目的の期が無ければ **null を返す**(近い列で代用しない)。

**単位も会社が自分で書いていた。**

- TSMC: `(Unit: NT$ million, except for EPS)`
- ASML: `(Figures in millions of euros unless otherwise indicated)`

書いていない表は読まない。倍率を推測すると桁が 1000 倍ずれる。

## 読めた値(実測)

| | TSMC 2Q26 | ASML Q2 2026 |
|---|---|---|
| 売上 | NT$ 1,270,381 M | € 9,326 M |
| 売上総利益 | NT$ 860,311 M | € 5,035 M |
| 営業利益 | NT$ 766,603 M | (表に無い) |
| 純利益 | NT$ 706,562 M | € 2,918 M |

数字は行ごと(`sourceText`)で保持している。数字だけを持ち出すと出典が付けられない。

## ★ 行き止まり: 四半期は現地通貨しか無い

**年次(20-F / XBRL)は USD で取れる**。TSMC が自分で USD 換算値を併記しているため。
**しかし四半期(6-K のプレスリリース)は TWD / EUR しか無い。**

いまの指標パイプラインは `selectBestFact` が **USD だけを通す**設計で、
盤面・比較・整形もすべてそれに乗っている。ここに TWD を流すと invariant が壊れる。

選択肢は 3 つあり、**どれもプロダクトの判断が要る**:

1. **四半期は数字を出さず、本文(会話の材料)としてだけ使う**
   通貨の問題が起きない。オーナーが承認した「6-K のプレスリリースを本文に使う」に一致する。
   盤面は年次 USD のまま。会話は最新四半期に答えられるようになる
2. **現地通貨のまま盤面に出す**
   1 社ずつ通貨が違う盤面になる。USD だけを前提にしている箇所を全部直す必要がある
3. **為替で USD に換算する**
   **出典の無い数字になる。** このアプリの建て付けと正面から矛盾するので推奨しない

**現状は 1 を前提に部品だけ作ってある**(表は読めるが、まだどこにも繋いでいない)。
繋ぎ先を決めるのはオーナー。

## 実装済み / 未実装

- [x] どの 6-K が四半期業績か判別(`six-k.ts`)
- [x] 損益表から期・通貨・単位・各行の値と原文を読む(`quarterly-results.ts`)
- [ ] 6-K の添付一覧を辿って本文を取ってくる経路
- [ ] 取り込み(`FilingCacheRecord` にどう載せるか)= **上の 1/2/3 の判断待ち**
- [ ] Shell は 6-K に XBRL があるので別経路
