...
var WatchlistAddRequestSchema = external_exports.object({
  ticker: external_exports.string().trim().min(1).max(16)
});
var ChatRequestSchema = external_exports.object({
  filingKey: external_exports.string().min(1),
  question: external_exports.string().trim().min(1).max(1e3)
});
var BillingSyncRequestSchema = external_exports.object({
  originalTransactionId: external_exports.string().trim().min(1),
  productId: external_exports.string().trim().min(1).optional(),
  active: external_exports.boolean().default(false)
});
var SearchQuerySchema = external_exports.object({
  q: external_exports.string().trim().min(1).max(100)
});
var SourceSchema = external_exports.object({
  sourceId: external_exports.string(),
  sectionType: external_exports.enum(["md_a", "xbrl_metric"]),
  sourceLabel: external_exports.string(),
  excerpt: external_exports.string()
});
var SummaryResponseSchema = external_exports.object({
  verdict: external_exports.string(),
  highlights: external_exports.array(
    external_exports.object({
      text: external_exports.string(),
      sourceIds: external_exports.array(external_exports.string()).min(1)
    })
  ),
...
var ChatModelResponseSchema = external_exports.object({
...
async function generateSummary(env, input) {
...
async function generateChatAnswer(env, input) {
  if (!env.GEMINI_API_KEY) {
...
__name(generateChatAnswer, "generateChatAnswer");
async function invokeGemini(env, prompt, kind) {
...
__name(invokeGemini, "invokeGemini");
function localSummaryFallback(input) {
  ...
  const headlineMetric = input.metrics.find((metric) => metric.yoyPercent !== void 0) ?? input.metrics[0];
  const verdict = headlineMetric ? `${input.companyName}\u306E\u6700\u65B0${input.formType}\u3067\u306F\u3001${metricLabel(
    headlineMetric.logicalName
  )}\u3092\u4E2D\u5FC3\u306B\u63D0\u51FA\u8CC7\u6599\u30D9\u30FC\u30B9\u3067\u78BA\u8A8D\u3067\u304D\u307E\u3059\u3002` : `${input.companyName}\u306E\u6700\u65B0${input.formType}\u3092\u65E5\u672C\u8A9E\u3067\u78BA\u8A8D\u3067\u304D\u307E\u3059\u3002`;
  return {
    verdict,
...
__name(localSummaryFallback, "localSummaryFallback");
function localChatFallback(input) {
  const questionTerms = input.question.toLowerCase().split(/\s+/).filter((term) => term.length >= 2);
  const scored = input.filing.sourceChunks.map((source) => ({
    source,
...
__name(localChatFallback, "localChatFallback");
...
// src/clients/sec-fetcher.ts
async function fetchTickerSnapshotFromFetcher(env) {
  return fetcherRequest(env, "/internal/sec/tickers-snapshot", {});
}
__name(fetchTickerSnapshotFromFetcher, "fetchTickerSnapshotFromFetcher");
async function fetchSubmissionsFromFetcher(cik, env) {
  return fetcherRequest(env, "/internal/sec/submissions", { cik });
}
__name(fetchSubmissionsFromFetcher, "fetchSubmissionsFromFetcher");
async function fetchFilingHtmlFromFetcher(filing, env) {
...
__name(fetchFilingHtmlFromFetcher, "fetchFilingHtmlFromFetcher");
async function fetchMetricsFromFetcher(cik, tags, env) {
  return fetcherRequest(env, "/internal/sec/metrics", { cik, tags });
}
__name(fetchMetricsFromFetcher, "fetchMetricsFromFetcher");
async function fetcherRequest(env, path, payload) {
  if (!env.SEC_FETCHER_BASE_URL) {
    throw new Error("SEC fetcher base URL is not configured");
  }
  ...
  if (!response.ok) {
    throw new Error(`SEC fetcher request failed (${response.status}) for ${path}: ${text}`);
  }
  return JSON.parse(text);
}
__name(fetcherRequest, "fetcherRequest");
...
async function searchTickers(query2, env) {
...
__name(searchTickers, "searchTickers");
function sortTickerSearchResults(items, normalizedQuery) {
  return items.map((item) => ({
    item,
    score: scoreTickerSearch(item, normalizedQuery)
  })).filter((candidate) => candidate.score !== null).sort((left, right) => {
...
__name(sortTickerSearchResults, "sortTickerSearchResults");
function scoreTickerSearch(item, query2) {
  const ticker = item.ticker.toLowerCase();
  const companyName = item.companyName.toLowerCase();
  if (ticker === query2) {
    return 0;
  }
  if (ticker.startsWith(query2)) {
    return 1;
  }
  if (companyName === query2) {
    return 2;
  }
  if (companyName.startsWith(query2)) {
    return 3;
  }
  if (ticker.includes(query2)) {
    return 4;
  }
  if (companyName.includes(query2)) {
    return 5;
  }
  return null;
}
__name(scoreTickerSearch, "scoreTickerSearch");
async function refreshTickerSnapshot(env) {
  const payload = await fetchTickerSnapshotFromFetcher(env);
  const snapshot = normalizeTickerSnapshot(payload, (/* @__PURE__ */ new Date()).toISOString());
  await env.KABUYOMI_CACHE.put("tickers_snapshot", JSON.stringify(snapshot));
}
__name(refreshTickerSnapshot, "refreshTickerSnapshot");
async function lookupTicker(ticker, env) {
...
__name(lookupTicker, "lookupTicker");
async function fetchSubmissions(cik, env) {
  return fetchSubmissionsFromFetcher(cik, env);
}
__name(fetchSubmissions, "fetchSubmissions");
function pickLatestSupportedFiling(tickerRecord, submissions) {
...
__name(pickLatestSupportedFiling, "pickLatestSupportedFiling");
function pickComparisonFiling(tickerRecord, submissions, current) {
  ...
  const sorted = candidates.map((item) => ({
    filing: item,
    distance: Math.abs(new Date(item.periodOfReport).getTime() - approxPriorYear.getTime())
  })).sort((left, right) => left.distance - right.distance);
  return sorted[0]?.filing ?? null;
}
__name(pickComparisonFiling, "pickComparisonFiling");
function buildFilingKey(extractorVersion, filing) {
  return `${extractorVersion}:${filing.cik}:${accessionWithoutDashes(filing.accessionNumber)}`;
}
__name(buildFilingKey, "buildFilingKey");
function buildPrimaryDocumentUrl(filing) {
  return `https://www.sec.gov/Archives/edgar/data/${Number(filing.cik)}/${accessionWithoutDashes(
    filing.accessionNumber
  )}/${filing.primaryDocument}`;
}
__name(buildPrimaryDocumentUrl, "buildPrimaryDocumentUrl");
async function fetchFilingHtml(filing, env) {
  const response = await fetchFilingHtmlFromFetcher(filing, env);
  return response.html;
}
__name(fetchFilingHtml, "fetchFilingHtml");
async function fetchMetricSnapshots(filing, comparisonFiling, env) {
  ...
  const results = [];
  for (const logicalName of Object.keys(METRIC_TAGS)) {
    const current = resolveFact(logicalName, filing, fetcherPayload.concepts, fetcherPayload.companyFacts);
    if (!current) {
      continue;
    }
    const comparison = comparisonFiling ? resolveFact(logicalName, comparisonFiling, fetcherPayload.concepts, fetcherPayload.companyFacts) : null;
    results.push({
      logicalName,
      ...
      yoyPercent: comparison && comparison.fact.val !== 0 ? (current.fact.val - comparison.fact.val) / Math.abs(comparison.fact.val) * 100 : void 0
    });
  }
  return results;
}
__name(fetchMetricSnapshots, "fetchMetricSnapshots");
function resolveFact(logicalName, filing, concepts, companyFacts) {
...
__name(resolveFact, "resolveFact");
function selectBestFact(units, filing, isEPS) {
  if (!units) {
    return null;
  }
  ...
  for (const [unit, facts] of Object.entries(units)) {
    const unitWeight = preferredUnits.includes(unit) ? 0 : 5e7;
    for (const fact of facts) {
      ...
      const score = unitWeight + Math.abs(endTime - targetPeriod) + Math.abs(filedTime - targetFiled);
...
__name(selectBestFact, "selectBestFact");
function allSupportedFilings(tickerRecord, submissions) {
  const recent = submissions.filings.recent;
  const filings = [];
  for (let index = 0; index < recent.form.length; index += 1) {
...
__name(allSupportedFilings, "allSupportedFilings");
...
function accessionWithoutDashes(accessionNumber) {
  return accessionNumber.replaceAll("-", "");
}
__name(accessionWithoutDashes, "accessionWithoutDashes");
function normalizeTickerSnapshot(raw, updatedAt = (/* @__PURE__ */ new Date()).toISOString()) {
...
__name(normalizeTickerSnapshot, "normalizeTickerSnapshot");
async function getTickerSnapshot(env) {
...
__name(getTickerSnapshot, "getTickerSnapshot");
...
var loopSegment = /* @__PURE__ */ __name(({ [NEXT]: next, [END]: end }, json2) => {
  while (next !== end) {
...
var attrAsJSON = /* @__PURE__ */ __name((attr, json2) => {
  json2.push(ATTRIBUTE_NODE, attr.name);
  const value = attr[VALUE].trim();
  if (value)
    json2.push(value);
}, "attrAsJSON");
var characterDataAsJSON = /* @__PURE__ */ __name((node, json2) => {
  const value = node[VALUE];
  if (value.trim())
    json2.push(node.nodeType, value);
}, "characterDataAsJSON");
var nonElementAsJSON = /* @__PURE__ */ __name((node, json2) => {
  json2.push(node.nodeType);
  loopSegment(node, json2);
}, "nonElementAsJSON");
var documentTypeAsJSON = /* @__PURE__ */ __name(({ name, publicId, systemId }, json2) => {
...
var elementAsJSON = /* @__PURE__ */ __name((element, json2) => {
...
// src/extractors/mda.ts
var TOKEN_BUDGET = 15e3;
var MIN_SECTION_CHARS = 2400;
...
__name(normalizeFilingText, "normalizeFilingText");
function extractMDASection(html, formType) {
  ...
  for (const [index, startMatch] of startMatches.entries()) {
    ...
    if (nextStart && nextStart.index - startMatch.index < 1e3) {
      continue;
    }
...
__name(extractMDASection, "extractMDASection");
...
__name(findAllMatches, "findAllMatches");
...
__name(trimToTokenBudget, "trimToTokenBudget");

// src/lib/pipeline.ts
function readQuotaIdentity(request) {
  const quotaSubject = request.headers.get("x-quota-subject");
  if (quotaSubject?.startsWith("pro:")) {
    return { quotaSubject, plan: "pro" };
  }
  const deviceKey = request.headers.get("x-device-key")?.trim() || "anonymous";
  return {
    quotaSubject: `free:${deviceKey}`,
    plan: "free"
  };
}
__name(readQuotaIdentity, "readQuotaIdentity");
async function ensureLatestFiling(ticker, env, config, options = {}) {
...
__name(ensureLatestFiling, "ensureLatestFiling");
async function loadFilingByKey(filingKey, env) {
...
__name(loadFilingByKey, "loadFilingByKey");
async function consumeChatQuota(identity, env, config) {
  return mutateUsage(identity, env, config, "consumeChat");
}
__name(consumeChatQuota, "consumeChatQuota");
async function consumeStockQuota(identity, env, config) {
  return mutateUsage(identity, env, config, "consumeStock");
}
__name(consumeStockQuota, "consumeStockQuota");
async function loadUsage(identity, env, config) {
  return mutateUsage(identity, env, config, "state");
}
__name(loadUsage, "loadUsage");
async function buildChatResponse(filing, question, env) {
  ...
  if (approvedSourceIds.length === 0) {
    throw new Error("Model returned no valid sourceIds");
  }
  return {
...
function buildCacheKey(extractorVersion, cik, accessionNumber) {
  return `filing_cache:${extractorVersion}:${cik}:${accessionNumber.replaceAll("-", "")}`;
}
__name(buildCacheKey, "buildCacheKey");
function buildTickerAliasKey(extractorVersion, ticker) {
  return `latest_filing_by_ticker:${extractorVersion}:${ticker.toUpperCase()}`;
}
__name(buildTickerAliasKey, "buildTickerAliasKey");
async function loadCachedLatestFiling(ticker, env, config) {
...
__name(loadCachedLatestFiling, "loadCachedLatestFiling");
async function ingestFiling(filing, comparisonFiling, env, config) {
...
__name(ingestFiling, "ingestFiling");
function buildSourceChunks(filing, mdaText, metrics) {
  const chunks = [];
  const mdParagraphs = mdaText.split(/\n{2,}/).map((paragraph) => paragraph.trim()).filter(Boolean);
  let mdOffset = 0;
  let sourceIndex = 1;
  for (const paragraph of mdParagraphs) {
    chunks.push({
      ...
      endOffset: mdOffset + paragraph.length,
  ...
  for (const metric of metrics) {
    chunks.push({
      ...
      sectionTitle: metricLabel2(metric.logicalName),
      sourceLabel: `XBRL ${metricLabel2(metric.logicalName)} (${metric.tagUsed})`,
      text: [
        `${metricLabel2(metric.logicalName)}: ${metric.value} ${metric.unit}`,
        metric.comparisonValue !== void 0 ? `\u6BD4\u8F03\u5024: ${metric.comparisonValue}` : null,
        metric.yoyPercent !== void 0 ? `YoY: ${metric.yoyPercent.toFixed(1)}%` : null
      ].filter(Boolean).join(" / "),
      startOffset: 0,
      endOffset: 0,
...
__name(buildSourceChunks, "buildSourceChunks");
...
__name(metricLabel2, "metricLabel");
async function acquireFilingLock(filingKey, env) {
...
__name(acquireFilingLock, "acquireFilingLock");
async function mutateUsage(identity, env, config, action) {
  ...
  if (!response.ok || !payload.usage) {
    ...
    throw new Error(payload.error ?? "Quota request failed");
  }
  return payload.usage;
}
__name(mutateUsage, "mutateUsage");
...
__name(loadRemoteConfig, "loadRemoteConfig");
...
// src/durable/filing-lock.ts
var FilingLockDO = class {
  constructor(state) {
    this.state = state;
  }
  static {
    __name(this, "FilingLockDO");
  }
  async fetch(request) {
  ...
  async acquire() {
    while (true) {
      const now = Date.now();
      const lockedUntil = await this.state.storage.get("lockedUntil") ?? 0;
      if (lockedUntil <= now) {
        await this.state.storage.put("lockedUntil", now + 3e4);
        return new Response(JSON.stringify({ ok: true }), {
          headers: { "content-type": "application/json" }
        });
      }
...
// src/durable/sec-rate-limiter.ts
var WINDOW_MS = 1e3;
var MAX_REQUESTS = 10;
var SecRateLimiterDO = class {
  constructor(state) {
    this.state = state;
  }
  static {
    __name(this, "SecRateLimiterDO");
  }
  async fetch(request) {
    const url = new URL(request.url);
  ...
  async take(tokens) {
    while (true) {
      const now = Date.now();
      const existing = (await this.state.storage.get("timestamps") ?? []).filter(
...
// src/durable/user-quota.ts
var UserQuotaDO = class {
  constructor(state) {
    this.state = state;
  }
  static {
    __name(this, "UserQuotaDO");
  }
  async fetch(request) {
    const body = await request.json();
    const key2 = `${body.dateJST}:${body.quotaSubject}`;
    const current = await this.state.storage.get(key2) ?? {
      plan: body.plan,
    ...
    if (body.action === "consumeChat") {
      if (current.chatsUsed >= current.chatLimit) {
        return this.reply({ error: "Daily chat quota exceeded", usage: current }, 429);
      }
      current.chatsUsed += 1;
    }
    if (body.action === "consumeStock") {
      if (current.stocksUsed >= current.stockLimit) {
        return this.reply({ error: "Watchlist limit exceeded", usage: current }, 429);
      }
      current.stocksUsed += 1;
    }
    current.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
    await this.state.storage.put(key2, current);
    return this.reply({ usage: current }, 200);
  }
  ...
  async fetch(request, env) {
    try {
      ...
      if (request.method === "POST" && url.pathname === "/v1/watchlist/add") {
        ...
        const identity = readQuotaIdentity(request);
        const usage = await consumeStockQuota(identity, env, config);
      ...
      if (request.method === "POST" && url.pathname === "/v1/billing/sync") {
        const parsed = BillingSyncRequestSchema.safeParse(await request.json());
        if (!parsed.success) {
          return badRequest("Invalid billing payload");
        }
        const stub = env.ENTITLEMENT.getByName("storekit");
...
function serializeCompanyResponse(filing) {
  return {
...
__name(serializeCompanyResponse, "serializeCompanyResponse");
export {
  EntitlementDO,
  FilingLockDO,
  SecRateLimiterDO,
